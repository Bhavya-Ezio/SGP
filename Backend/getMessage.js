...
async function get_data(s_no, r_no) {
    ...
    try {
        const query = `SELECT m.msg_id as msg_id,m.sender_no, u1.username AS sender_name, m.receiver_no, u2.username AS receiver_name, m.message, m.translated_content FROM messages AS m LEFT JOIN login_info AS u1 ON m.sender_no = u1.mob_number LEFT JOIN login_info AS u2 ON m.receiver_no = u2.mob_number where (m.sender_no=$1 and m.receiver_no=$2) or (m.sender_no=$2 and m.receiver_no=$1);`;
